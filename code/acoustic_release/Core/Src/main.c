...
/* USER CODE BEGIN PV */
uint16_t buf[FFT_LEN];
uint16_t imag[FFT_LEN];
uint16_t mag[FFT_LEN];
uint8_t adcbuf_ready = 0;
/* USER CODE END PV */
...
/* USER CODE BEGIN 4 */
/* Interrupt called when DMA transfer is done -
 * set flag variable to start computation in main loop
 */
